...
- **长按磁贴**：呼出系统磁贴详情面板，点齿轮（⚙）打开 WebUI `http://127.0.0.1:2023`（服务监听 `0.0.0.0:2023`）
- 磁贴图标实时反映代理状态（亮 = 运行中，暗 = 已停止）

> 说明：Android 的 `TileService` 没有 `onLongClick` 钩子——长按磁贴由系统展示详情面板。模块把 `QS_TILE_PREFERENCES` 入口指到主界面，详情面板的齿轮一点即开 WebUI，这是原生框架下最接近"长按打开 WebUI"的实现。
